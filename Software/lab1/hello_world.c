...
int main()
{
	...
	for(i=0; i < 2500; i++)
	{
		...
		while( IORD( EGM_BASE, 1) == 1 )
		{
			// We've already counted how many background tasks we can fit into one
			// period, so we'll do one fewer to prioritize latency
			...
			// Reached the first upwards edge, so we'll respond and begin counting
			...
			// Reached the first downward edge
			...
			// Reached the second upward edge, don't run another background thingy
